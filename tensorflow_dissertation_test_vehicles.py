...
train_dir = os.path.join(base_dir, 'train_cars')
validation_dir = os.path.join(base_dir, 'validation_cars')
test_dir = os.path.join(base_dir, 'test_cars')
...
def read_and_process_image(list_of_images):
    ...
    for image in list_of_images:
        ...
        if 'cars' in image:
            y.append(1)
        elif 'pickup' in image:
            y.append(0)
    return X, y
        
# Directory with training butterfly pics
train_butterflies_dir = os.path.join(train_dir, 'pickups')

# Directory with training beetles pics
train_beetles_dir = os.path.join(train_dir, 'cars')

# Directory with validation butterfly pics
val_butterflies_dir = os.path.join(validation_dir, 'pickups')

# Directory with validation beetles pics
val_beetles_dir = os.path.join(validation_dir, 'cars')
...
for batch in test_datagen.flow(x, batch_size=1):
    ...
    if pred > 0.5:
        text_labels.append('Pick-up truck')
    else:
        text_labels.append('Car')
